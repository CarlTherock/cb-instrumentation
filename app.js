(() => {
  ...
  function snoozeAlarm() {
    ...
    stopTorch();
    const snapshot = getRecordedSnapshot();
    if (snapshot && audioContext && snapshot.length >= audioContext.sampleRate * 0.5) {
      if (confirm('Écouter les 20 dernières secondes avant l’alarme ?')) { playRecordedClip(); return; }
    }
  ...
  function playStoredRecording(id) {
    ...
    const ctx = audioContext; const bus = getAlarmBus(ctx);
    const buffer = ctx.createBuffer(1, rec.data.length, rec.sampleRate);
    buffer.copyToChannel(rec.data, 0);
    const source = ctx.createBufferSource(); source.buffer = buffer;
    source.connect(bus);
    source.onended = () => { if (!listening && audioContext === ctx) { try { ctx.close(); } catch {} audioContext = null; } };
    source.start();
