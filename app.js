(() => {
  ...
  function openPlayer(buffer) {
    ensureAudioContext(); audioContext.resume().catch(() => {});
  ...
  function playStoredRecording(id) {
    const rec = callRecordings.find(item => item.id === id);
    if (!rec) return;
    ensureAudioContext(); audioContext.resume().catch(() => {});
    openPlayer(bufferFromSnapshot(audioContext, rec.data, rec.sampleRate));
  }
  ...
  ui.snoozeListen.addEventListener('click', () => {
    const rec = callRecordings.find(item => item.id === lastTriggerId);
    ui.snoozePanel.hidden = true;
    if (rec) { ensureAudioContext(); audioContext.resume().catch(() => {}); openPlayer(bufferFromSnapshot(audioContext, rec.data, rec.sampleRate)); }
