(() => {
  ...
  const ui = {
    ...
    language: $('languageSelect'), cooldown: $('cooldownRange'), cooldownValue: $('cooldownValue'),
  ...
  let settings = load(SETTINGS_KEY, { language: 'fr-CA', cooldown: 15 });
  let events = load(HISTORY_KEY, []);
  let recognition, stream, audioContext, analyser, audioData, animationId, wakeLock, alarmOscillator, alarmGain;
  ...
  function silenceAlarm() { ui.alarmPanel.hidden = true; document.body.classList.remove('alarming'); stopAlarmSound(); }
  function playAlarm() {
    ...
    osc.connect(gain).connect(ctx.destination); const t = ctx.currentTime;
    for (let i = 0; i < 8; i++) { gain.gain.setValueAtTime(0.0001, t + i * .25); gain.gain.linearRampToValueAtTime(.24, t + i * .25 + .025); gain.gain.setValueAtTime(.24, t + i * .25 + .15); gain.gain.linearRampToValueAtTime(.0001, t + i * .25 + .19); }
    osc.start(); osc.stop(t + 2.1); alarmOscillator = osc; alarmGain = gain;
  }
  function stopAlarmSound() { try { alarmOscillator?.stop(); } catch {} alarmOscillator = null; alarmGain = null; }
  ...
  ui.language.value = settings.language; ui.cooldown.value = settings.cooldown; ui.cooldownValue.textContent = `${settings.cooldown} s`;
